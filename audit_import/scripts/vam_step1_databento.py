...
def next_state(
    ...
    if current == State.BULL_100:
        ...
        reentry_ok = spy_close > spy_sma200 and vix < REENTRY_VIX
        if REENTRY_REQUIRE_50SMA:
            reentry_ok = reentry_ok and spy_close > spy_sma50
        if reentry_ok:
            return (
                State.BULL_100,
                f"RE-ENTRY: SPY>{spy_sma200:.0f}(200), VIX={vix:.1f}<{REENTRY_VIX}",
            )
...
def run_step1_backtest(df: pd.DataFrame) -> tuple[list[Trade], list[dict], dict]:
    ...
    trades: list[Trade] = []
    daily_log: list[dict] = []
    reentry_fresh_days: int | None = None  # days since last CASH→BULL_100; None when in/returning to CASH
    ...
    for date, row in trading_df.iterrows():
        ...
        if pending_trade is not None:
            ...
            state = new_state
            # Reset streak counter when entering BULL_100 from CASH so DEFENSIVE
            # requires a fresh 2-day breach while invested, not an inherited count.
            ...
            trades.append(
                {
                    ...
                    "overnight_gap_pct": round((upro_open / upro_price - 1) * 100, 2)
                    if upro_price > 0
                    else 0,
                    # ── Position before ──
                    ...
                    # ── Verification ──
                    "check_pv_equals_cash_plus_shares_x_close": round(
                        pv_after_close - (cash + shares * upro_price), 2
                    ),
                    "check_alloc_pct_actual": round((shares * upro_open / pv_before) * 100, 2)
                    if pv_before > 0
                    else 0,
                }
            )
        ...
        # Increment days-since-reentry and cap below_streak so DEFENSIVE needs
        # a fresh consecutive breach while invested, not an inherited pre-entry count.
        if reentry_fresh_days is not None:
            reentry_fresh_days += 1
            below_streak = min(below_streak, reentry_fresh_days)
        ...
        defensive_trigger = below_streak >= DEFENSIVE_CONFIRM_DAYS
        ...
        daily_log.append(
            {
                ...
                "upro_daily_return_pct": round((upro_price / upro_open - 1) * 100, 2)
                if upro_open > 0
                else 0,
                ...
                "kill_reason": ("VIX>" + str(VIX_KILL) if vix_kill_active else "")
                ...
                "defensive_trigger": "YES"
                if defensive_trigger
                else f"NO (need {DEFENSIVE_CONFIRM_DAYS}d, have {below_streak}d)",
