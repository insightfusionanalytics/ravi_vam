...
7-State Machine (per IFA_Ravi_Proposal_v2):
  BULL_100      — 75% UPRO + 25% TQQQ (SPY & QQQ above 50-SMA, VIX < 30)
  BULL_TRIMMED  — 75% of bull allocation (RSI > 75 trim fired)
  DEFENSIVE_SPY — SPY below 50-SMA: UPRO sleeve halved, TQQQ sleeve intact
  DEFENSIVE_QQQ — QQQ below 50-SMA: TQQQ sleeve halved, UPRO sleeve intact
  DEFENSIVE_BOTH — Both below 50-SMA: both sleeves halved
  CASH          — Kill switch: 100% cash (VIX > 30 OR SPY < 200-SMA)
  SMA_RECOVERY  — Exiting CASH: SPY > 200-SMA AND VIX < 30, awaiting full bull conditions
...
Yahoo Finance data is split-adjusted and has different price levels.
DataBento provides raw unadjusted data — splits are applied in-memory.
Using Yahoo data will produce WRONG results. All client-facing backtests
must use DataBento CSVs from data/databento/ and data/cboe/ ONLY.
"""
...
# Allocations: (UPRO_pct, TQQQ_pct) — cap at 99% invested in bull mode
# BULL_100: 74.25% UPRO + 24.75% TQQQ = 99% invested (75/25 split, 1% reserved for costs)
# BULL_TRIMMED: sell 25% of total to cash = 56.25% UPRO + 18.75% TQQQ
# DEFENSIVE_SPY: UPRO sleeve halved = 37.5% UPRO + 25% TQQQ
# DEFENSIVE_QQQ: TQQQ sleeve halved = 75% UPRO + 12.5% TQQQ
# DEFENSIVE_BOTH: both sleeves halved = 37.5% UPRO + 12.5% TQQQ
# SMA_RECOVERY: cautious re-entry (75% of bull) = 56.25% UPRO + 18.75% TQQQ
...
REENTRY_REQUIRE_50SMA = False # Flag 6: Disabled — re-entry on SPY > 200-SMA confirmed by Ravi (feb10 00:13:35).
...
VIX_KILL = 30.0              # Kill switch VIX threshold (confirmed: mar1 00:38:10)
RSI_SELL = 75.0              # RSI trim trigger (confirmed: feb10 00:09:38)
SLIPPAGE_BPS_NORMAL = 5.0    # Normal slippage bps (confirmed: feb10 00:04:08)
SLIPPAGE_BPS_STRESS = 20.0   # Stress slippage bps (VIX >= 25 — IFA addition)
...
def next_state(
    ...
    """
    7-state machine with independent SPY/QQQ defensive triggers and SMA_RECOVERY.
    Priority: Kill > Defensive > RSI
    States: BULL_100, BULL_TRIMMED, DEFENSIVE_SPY, DEFENSIVE_QQQ, DEFENSIVE_BOTH, CASH, SMA_RECOVERY
    """
    # PRIORITY 1: Kill Switch (from any non-CASH state, including SMA_RECOVERY)
    ...
    # CASH — initial re-entry trigger: SPY > 200-SMA AND VIX < REENTRY_VIX
    if current == State.CASH:
        if spy_close > spy_sma200 and vix < REENTRY_VIX:
            return State.SMA_RECOVERY, f"SMA_RECOVERY: SPY>{spy_sma200:.0f}(200SMA), VIX={vix:.1f}<{REENTRY_VIX}"
        return State.CASH, "HOLD"

    # SMA_RECOVERY — awaiting full bull conditions to transition to BULL_100
    if current == State.SMA_RECOVERY:
        if REENTRY_REQUIRE_50SMA:
            ...
            reentry_reason = f"RE-ENTRY COMPLETE: VIX={vix:.1f}<{REENTRY_VIX} (50-SMA check skipped)"
        if reentry_ok:
...
def run_step2_backtest(df: pd.DataFrame) -> tuple[list[dict], list[dict], dict]:
    ...
    trades: list[dict] = []
    daily_log: list[dict] = []
    reentry_fresh_days: int | None = None  # days since last re-entry from CASH; None when in/returning to CASH
    ...
    for date, row in trading_df.iterrows():
        ...
        if pending_trade is not None:
            ...
            state = new_state
            # Reset streak counter when entering BULL_100 from CASH or SMA_RECOVERY
            # so DEFENSIVE requires a fresh 2-day breach while invested.
            if new_state == State.BULL_100 and old_state_val in (State.CASH.value, State.SMA_RECOVERY.value):
                reentry_fresh_days = 0
            elif new_state == State.CASH:
                reentry_fresh_days = None
                ...
                trades.append(
                    {
                        ...
                        "overnight_gap_pct": round((upro_exec / upro_price - 1) * 100, 2)
                        if upro_price > 0
                        else 0,
                        # ── Position before ──
                        ...
                        "check_pv_equals_cash_plus_positions": round(
                            pv_at_close
                            - (cash + upro_shares * upro_price + tqqq_shares * tqqq_price),
                            2,
                        ),
                ...
                trades.append(
                    {
                        ...
                        "overnight_gap_pct": round((tqqq_exec / tqqq_price - 1) * 100, 2)
                        if tqqq_price > 0
                        else 0,
                        # ── Position before ──
        ...
        # Increment days-since-reentry and cap both below streaks so DEFENSIVE
        # requires a fresh consecutive breach while invested, not an inherited count.
        if reentry_fresh_days is not None:
            reentry_fresh_days += 1
            spy_below_streak = min(spy_below_streak, reentry_fresh_days)
            qqq_below_streak = min(qqq_below_streak, reentry_fresh_days)
        ...
        if KILL_SWITCH_LOGIC == "OR":
            ...
            kill_switch_on = vix_kill_active and spy_below_200
        spy_defensive_trigger = spy_below_streak >= DEFENSIVE_CONFIRM_DAYS
        qqq_defensive_trigger = qqq_below_streak >= DEFENSIVE_CONFIRM_DAYS
        ...
        daily_log.append(
            {
                ...
                if (len(trades) > 0 and trades[-1]["execution_date"] == date.strftime("%Y-%m-%d"))
                else "NO",
                ...
                "spy_defensive_trigger": "YES"
                if spy_defensive_trigger
                else f"NO (need {DEFENSIVE_CONFIRM_DAYS}d, have {spy_below_streak}d)",
                ...
                "qqq_defensive_trigger": "YES"
                if qqq_defensive_trigger
                else f"NO (need {DEFENSIVE_CONFIRM_DAYS}d, have {qqq_below_streak}d)",
                # ── Signal 4: RSI ──
                "spy_rsi_14": round(spy_rsi, 2),
                "rsi_zone": "HOT (>75)"
                if rsi_hot
                else ("COOL (<60)" if rsi_cool else "NEUTRAL (60-75)"),
                # ── Portfolio ──
                ...
                "daily_pnl": round(
                    portfolio_value
                    - (daily_log[-1]["portfolio_value"] if daily_log else INITIAL_CAPITAL),
                    2,
                ),
...
def main() -> dict:
    ...
    print("\n[Step 2] Running 7-state backtest...")
