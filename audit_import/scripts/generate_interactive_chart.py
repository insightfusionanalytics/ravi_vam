    ...
    states = [
        {
            "time": r["date"],
            "state": r["state"],
            "color": state_colors.get(r["state"], "rgba(128,128,128,0.1)"),
        }
...
document.getElementById('stat-cagr').textContent = '+' + cagr.toFixed(1) + '%';
...
const spyFirst = firstDay.spy;
const spyLast = lastDay.spy;
const spyCagr = (Math.pow(spyLast/spyFirst, 1/years) - 1) * 100;
const alpha = cagr - spyCagr;
document.getElementById('stat-alpha').textContent = '+' + alpha.toFixed(1) + '%';
...
def main() -> None:
    ...
    output_path = DELIVERY_DIR / "vam_step1_interactive_chart.html"
